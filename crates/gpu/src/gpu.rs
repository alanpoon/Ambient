...
#[derive(Debug)]
pub struct OptionGpuKey;
impl SyncAssetKey<Arc<Mutex<OptionGpu>>> for OptionGpuKey {}

#[derive(Debug)]
pub struct OptionGpu{
...
impl OptionGpu{
    pub fn set(&mut self,window: Option<&Window>,device:&wgpu::Device,adapter: &wgpu::Adapter,instance:&wgpu::Instance,settings:&RenderSettings){
        let surface = window
        .map(|window| unsafe { instance.create_surface(window) })
        .transpose()
        .context("Failed to create surface").unwrap();
        ...
        self.swapchain_mode = swapchain_mode;
    }
    pub fn resize(&self, size: winit::dpi::PhysicalSize<u32>,device:&wgpu::Device) {
        if let Some(surface) = &self.surface {
            if size.width > 0 && size.height > 0 {
                surface.configure(&device, &self.sc_desc(uvec2(size.width, size.height)));
            }
        }
    }
